...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 2:
        print
        "Calling syntax: daltonize.py [fullpath to image file]"
        print
        "Example: daltonize.py C:/wikiinstance/data/pages/PageName/attachments/pic.png"
        sys.exit(1)
    ...
    for col_deficit in ['d', 'p', 't']:
        print
        "Creating %s corrected version" % colorblindness[col_deficit]
        modified_filename, modified_fpath = execute(fname, sys.argv[1], col_deficit)
        if modified_fpath == sys.argv[1]:
            print
            "Error while processing image: PIL/NumPy missing and/or source file is a grayscale image."
        else:
            print
